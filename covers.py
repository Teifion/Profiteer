...
def get_coverage(suite, test_program, root_dir, verbose = True, module_skip = [], dirs_skip = [], output_mode="print"):
    modules = OrderedDict()
...
def get_tests(test_program):
    """Get all the tests we ran, then we can work out what we didn't cover"""
    tests = []
    
    # it should be a unittest.TestCase
    if type(test_program) != unittest.TestSuite:
        return [test_program]
...
def get_test_targets(test_program):
    tests = get_tests(test_program)
    targets = set()
