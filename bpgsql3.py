...
# Modified by Teifion to work with Python3
...
def _time_to_python(timepart):
	...
	"""
	str_timepart = timepart.decode('utf-8')
	
	if '+' in str_timepart:
		str_timepart, tz = str_timepart.split('+')
		tz = _SimpleTzInfo(tz)
	elif '-' in str_timepart:
		str_timepart, tz = str_timepart.split('-')
		tz = _SimpleTzInfo('-' + tz)
	else:
		tz = None

	hour, minute, second = str_timepart.split(':')
	if '.' in second:
