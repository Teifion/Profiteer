import unittest
from profiteer import math_f

class Trigonometry_tester(unittest.TestCase):
	test_targets = [
		math_f.add_vectors,
		math_f.move_to_vector,
		math_f._move_to_vector_2d,
		math_f.distance,
		math_f.angle,
		math_f.angle_diff,
		math_f.get_midpoint,
	...
	def test_bound_angle(self):
		...
		for a, expected in vals:
			self.assertEqual(expected, math_f.bound_angle(a))
	...
	def test_add_vectors(self):
		...
		for a, b, expected in vals:
			self.assertEqual(math_f.add_vectors(a, b), expected)
	...
	def test_move_to_vector(self):
		...
		for angle, distance, expected in vals:
			answer = math_f.move_to_vector(angle, distance)
	...
	def test_distance(self):
		...
		for a, b, expected in vals:
			self.assertAlmostEqual(math_f.distance(a, b), expected, places=2)
	...
	def test_angle_diff(self):
		...
		for a1, a2, expected in vals:
			self.assertEqual(math_f.angle_diff(a1, a2), expected)
	...
	def test_angle(self):
		...
		for a, b, expected in vals:
			r, r2 = math_f.angle(a, b)
			self.assertAlmostEqual(r, expected[0], places=2, msg="math_f.angle(%s, %s) should equal %s, instead got %s" % (
				a, b, expected[0], r
			))
	...
	def test_midpoint(self):
		...
		for pos1, pos2, distance, expected in vals:
			x,y,z = math_f.get_midpoint(pos1, pos2, distance)
